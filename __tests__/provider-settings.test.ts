...
beforeEach(() => {
  vi.resetModules();
  vi.clearAllMocks();
  delete process.env.PILLAR_PRESS_HOSTED_SECRET_KEY;
  delete process.env.PILLAR_PRESS_ENCRYPTION_KEY;
  delete process.env.AUTH_SECRET;
});

describe("hosted provider setting encryption", () => {
  it("encrypts hosted provider keys without exposing the raw value", async () => {
    process.env.PILLAR_PRESS_HOSTED_SECRET_KEY = "test-hosted-secret";
    ...
    expect(decryptHostedSecret(encrypted, { PILLAR_PRESS_HOSTED_SECRET_KEY: "wrong-secret" } as unknown as NodeJS.ProcessEnv)).toBeUndefined();
  });
});
