...
beforeEach(() => {
  ...
  delete process.env.PILLAR_PRESS_HOSTED_WEB;
  delete process.env.PILLAR_PRESS_RUNTIME;
...
describe("hosted billing session audit events", () => {
  ...
  it("records sanitized audit metadata when creating a Checkout session", async () => {
    ...
    vi.doMock("@/lib/billing/stripe", () => ({
      ...
      appBaseUrl: vi.fn(() => "https://pillarpress.test"),
      getStripe: vi.fn(() => ({
        checkout: {
          sessions: {
    ...
    const res = await POST(new Request("https://pillarpress.test/api/billing/checkout", {
      method: "POST",
  ...
  it("records sanitized audit metadata when creating a Customer Portal session", async () => {
    ...
    const res = await POST(new Request("https://pillarpress.test/api/billing/portal", {
      method: "POST",
    ...
    expect(createPortal).toHaveBeenCalledWith({
      customer: "cus_123",
      return_url: "https://pillarpress.test/?billing=portal-return",
    });
    expect(getOrCreateBillingCustomer).toHaveBeenCalledWith({
      workspaceId: "workspace_1",
      userId: "user_1",
