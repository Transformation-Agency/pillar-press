...
describe("browser hosted auth client", () => {
  it("signs in, stores the hosted session, and loads the same-origin workspace session with a bearer token", async () => {
    ...
    const window = {
      ...
      location: {
        origin: "https://app.pillarpress.test",
        pathname: "/",
        search: "",
        hash: "",
        href: "https://app.pillarpress.test/",
      },
  ...
  it("creates a hosted account with an explicit redirect and reports confirmation-required signup", async () => {
    ...
    const fetch = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url === "https://example.supabase.co/auth/v1/signup?redirect_to=https%3A%2F%2Fapp.pillarpress.test%2F") {
        expect(init?.method).toBe("POST");
        expect(JSON.parse(String(init?.body))).toEqual({
    ...
    expect(calls.map((call) => call.url)).toEqual([
      "/api/auth/config",
      "https://example.supabase.co/auth/v1/signup?redirect_to=https%3A%2F%2Fapp.pillarpress.test%2F",
    ]);
  });
  ...
  it("requests a hosted password reset email with an explicit redirect", async () => {
    ...
    const fetch = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url === "https://example.supabase.co/auth/v1/recover?redirect_to=https%3A%2F%2Fapp.pillarpress.test%2F") {
        expect(init?.method).toBe("POST");
    ...
    expect(calls.map((call) => call.url)).toEqual([
      "/api/auth/config",
      "https://example.supabase.co/auth/v1/recover?redirect_to=https%3A%2F%2Fapp.pillarpress.test%2F",
    ]);
  });
  ...
  it("completes a recovery-link password update and hydrates the hosted session", async () => {
    ...
    const window = {
      ...
      location: {
        ...
        href: "https://app.pillarpress.test/#type=recovery&access_token=recovery-access-token",
      },
      history: { replaceState: vi.fn() },
      dispatchEvent: vi.fn(),
    };
    ...
    expect(window.history.replaceState).toHaveBeenCalledWith(null, "Pillar Press", "/");
    expect(calls.map((call) => call.url)).toEqual([
      "/api/auth/config",
      "/api/auth/session",
