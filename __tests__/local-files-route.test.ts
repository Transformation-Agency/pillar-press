...
beforeEach(() => {
  vi.resetModules();
  storageDir = join(tmpdir(), `pillar-press-local-files-${Date.now()}-${Math.random().toString(16).slice(2)}`);
...
async function getLocalFile(path: string[]) {
  ...
  return GET(new Request("https://pillarpress.test/api/local-files/media/example.txt?contentType=text/plain"), {
