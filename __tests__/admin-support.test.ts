...
beforeEach(() => {
  ...
  vi.clearAllMocks();
  [
    "PILLAR_PRESS_ADMIN_SECRET",
    "PILLAR_PRESS_SUPPORT_SECRET",
    ...
    "STRIPE_PRICE_PRO",
    "PILLAR_PRESS_HOSTED_SECRET_KEY",
    "PILLAR_PRESS_JOB_SECRET",
    "LLM_API_KEY",
    "OPENAI_API_KEY",
    "ANTHROPIC_API_KEY",
    "XAI_API_KEY",
...
describe("GET /api/admin/support/readiness", () => {
  ...
  it("reports missing hosted SaaS configuration without leaking secret values", async () => {
    process.env.PILLAR_PRESS_ADMIN_SECRET = "admin-secret";
    const safeRecordAuditEvent = vi.fn();
    vi.doMock("@/lib/audit", () => ({ safeRecordAuditEvent }));
    ...
    expect(body.product).toBe("pillar_press");
    ...
    expect(body.checks).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({
        id: "runtime",
        ok: false,
        missing: expect.arrayContaining(["PILLAR_PRESS_RUNTIME=hosted or PILLAR_PRESS_HOSTED_WEB=true"]),
      }),
    ]));
  ...
  it("returns ready when required hosted SaaS configuration is present", async () => {
    ...
    process.env.PILLAR_PRESS_HOSTED_WEB = "true";
    process.env.PILLAR_PRESS_LOCAL_FIRST = "false";
    process.env.DATA_BACKEND = "postgres";
    process.env.AUTH_DISABLED = "false";
    process.env.DATABASE_URL = "postgres://postgres:secret@db.example.com/pillar_press";
    ...
    process.env.PILLAR_PRESS_STORAGE = "supabase";
    process.env.APP_URL = "https://pillarpress.test";
    process.env.STRIPE_SECRET_KEY = "sk_live_secret";
    process.env.STRIPE_WEBHOOK_SECRET = "whsec_secret";
    process.env.STRIPE_PRICE_STARTER = "price_starter";
    process.env.STRIPE_PRICE_PRO = "price_pro";
    process.env.PILLAR_PRESS_HOSTED_SECRET_KEY = "encryption-key-value";
    process.env.PILLAR_PRESS_JOB_SECRET = "job-secret";
    const safeRecordAuditEvent = vi.fn();
    vi.doMock("@/lib/audit", () => ({ safeRecordAuditEvent }));

    const { GET } = await import("../app/api/admin/support/readiness/route");
    const res = await GET(new Request("http://test.local/api/admin/support/readiness", {
      headers: { "x-pillar-press-admin-secret": "admin-secret" },
    }));
    const body = await res.json();
...
describe("GET /api/admin/support/workspaces", () => {
  ...
  it("returns a detailed workspace support view with scrubbed metadata", async () => {
    process.env.PILLAR_PRESS_SUPPORT_SECRET = "support-secret";
    const safeRecordAuditEvent = vi.fn();
    const execute = vi
      .fn()
