...
describe("live provider input preflight", () => {
  it("reports env readiness without returning secret values", () => {
    const summary = summarizeLiveProviderInputs({
      PILLAR_PRESS_LIVE_OPENAI_API_KEY: "sk-live-placeholder",
      PILLAR_PRESS_LIVE_XAI_API_KEY: "xai-live-placeholder",
      PILLAR_PRESS_LIVE_PROVIDER_VERIFY_SPEND_CREDITS: "yes",
      PILLAR_PRESS_LIVE_DESKTOP_SETTINGS_PATH: "/tmp/does-not-exist",
    });

    expect(summary.env.PILLAR_PRESS_LIVE_OPENAI_API_KEY).toBe(true);
    expect(summary.env.PILLAR_PRESS_LIVE_XAI_API_KEY).toBe(true);
  ...
  it("detects saved desktop provider keys as saved-only readiness", () => {
    ...
    const summary = summarizeLiveProviderInputs({
      PILLAR_PRESS_LIVE_DESKTOP_SETTINGS_PATH: settingsPath,
    });
