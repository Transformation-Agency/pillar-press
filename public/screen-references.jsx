...
function References({ refs, role, campaignName }) {
  ...
  React.useEffect(() => {
    ...
    window.addEventListener("kingspress:store-persist", onPersist);
    return () => window.removeEventListener("kingspress:store-persist", onPersist);
  }, []);
        ...
        {hasDesktopBridge && (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(230px, 1fr))", gap: 12, marginTop: 18 }}>
            ...
            <section className="card" style={{ padding: 18 }}>
              ...
              <p className="muted" style={{ margin: 0, fontSize: 14, lineHeight: 1.55 }}>
                Use Help → Check for Updates for the latest King's Press DMG. Use Backup in the top bar before big changes; backups are local and secrets are redacted.
              </p>
            </section>
          </div>
        )}
