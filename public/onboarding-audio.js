...
(function () {
  ...
  function describeAudioError(error, fallback) {
    ...
    if (
      ...
      return "Microphone permission is blocked. Allow Pillar Press microphone access in macOS System Settings, then try again or keep typing.";
    }
    ...
    if (code === "audio-capture" || code === "audio capture" || code === "notreadableerror") {
      return "Pillar Press could not start the microphone. Check that another app is not using it, then try again or keep typing.";
    }
    if (code === "network") {
