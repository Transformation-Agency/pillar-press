...
import { resolveProviderConnection } from "@/lib/llm/config";
...
export function getMediaProviderStatus(env: NodeJS.ProcessEnv = process.env): MediaProviderStatus {
  ...
  const savedLLMOpenAI = resolveProviderConnection("openai", {}, env);
  const savedXai = desktopMediaProvider("xai", env);
  const savedCustomImage = desktopMediaProvider("custom-image", env);
  const openaiModels = imageModels("openai", splitModels(env.MEDIA_OPENAI_IMAGE_MODELS, ["gpt-image-1.5", "gpt-image-1-mini", "gpt-image-1"]));
  ...
  const openai: MediaProviderInfo = {
    id: "openai",
    label: "OpenAI",
    configured: hasValue(env.MEDIA_OPENAI_API_KEY) || hasValue(env.OPENAI_API_KEY) || hasValue(savedOpenAI?.apiKey) || hasValue(savedLLMOpenAI.apiKey),
