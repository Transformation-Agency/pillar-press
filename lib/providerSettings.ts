...
function hostedSecretKey(env: NodeJS.ProcessEnv = process.env) {
  const raw =
    trim(env.KINGS_PRESS_HOSTED_SECRET_KEY) ||
    trim(env.KINGS_PRESS_ENCRYPTION_KEY) ||
    trim(env.AUTH_SECRET);
  if (!raw) {
    throw new Error("Missing KINGS_PRESS_HOSTED_SECRET_KEY for hosted provider key encryption.");
  }
