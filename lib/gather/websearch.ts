...
import { braveSearchKey } from "./integrationKeys";

export async function runWebSearch(query: string, count = 5): Promise<GatherItem[]> {
  const key = braveSearchKey();
  if (!key) throw new GatherError(500, "config", "Brave Search isn't connected. Add a key under Gather → Integrations (or set BRAVE_SEARCH_API_KEY).");
