# Pillar Press Technical Flow Diagrams

This document maps the current Pillar Press codebase, not the older Pillar Press
...
Pillar Press now has two runtime modes in the same codebase:
...
Hosted override logic is in `lib/local/mode.ts`. `PILLAR_PRESS_RUNTIME=hosted`,
`PILLAR_PRESS_HOSTED_WEB=true`, `PILLAR_PRESS_LOCAL_FIRST=false`, or
...
providers directly. It calls same-origin Pillar Press API routes.
...
Pillar Press intentionally injects approved campaign preferences into production
