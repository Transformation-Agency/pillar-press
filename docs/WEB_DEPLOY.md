...
Pillar Press can run as an internet-accessible Next.js web app without reverting
...
- The current Pillar Press static workspace UI.
...
PILLAR_PRESS_HOSTED_SECRET_KEY=<long random encryption secret>
PILLAR_PRESS_JOB_SECRET=<long random worker secret>
```

With hosted auth enabled, the static browser app shows a Pillar Press
...
```bash
PILLAR_PRESS_HOSTED_SECRET_KEY=<long random value>
```
...
These routes are disabled unless `PILLAR_PRESS_ADMIN_SECRET` or
`PILLAR_PRESS_SUPPORT_SECRET` is set. Send the secret as `Authorization: Bearer
<secret>` or `x-pillar-press-admin-secret: <secret>`. Responses include
...
`PILLAR_PRESS_ADMIN_SECRET`; `PILLAR_PRESS_SUPPORT_SECRET` is read-only and cannot
...
Only trial subscriptions can be extended. The route records both a
`trial_events.extended` row and an admin audit event.
...
When `PILLAR_PRESS_HOSTED_WEB=true` or `PILLAR_PRESS_RUNTIME=hosted` and
...
curl -X POST "https://your-domain.example/api/jobs/run" \
  -H "Authorization: Bearer $PILLAR_PRESS_JOB_SECRET" \
