# King's Press Desktop UX Readiness Audit

Date: 2026-06-24
Scope: installed King's Press Editorial Desk desktop app plus first-run, setup, settings, local data, model/provider, and recovery paths in code and tests.
...
- Ran installed app: `/Applications/King's Press Editorial Desk.app`, bundle ID `com.kingspress.editorialdesk`.
...
3. Store hydration and background persistence can fail silently in the main UX.
   ...
   - Suggested copy: "King's Press could not refresh everything from local storage. Your local data was not deleted. Retry."
...
- Current documents are visible across focuses, including restored Pillar Brief archive documents.
...
2. First-run desktop smoke with isolated data dir:
   - Launch installed app or Tauri dev with `KINGS_PRESS_DESKTOP_DATA_DIR` pointing to a temp directory.
