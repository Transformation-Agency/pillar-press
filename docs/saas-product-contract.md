# Pillar Press SaaS Product Contract

This is the Stage 0 contract for converting the hosted Pillar Press web app into
...
Pillar Press should support both:

- BYOK: user/workspace stores encrypted provider keys server-side.
- Managed: Pillar Press uses platform-owned provider keys with strict quotas.
...
8. Stage 4: update onboarding and account/billing UI. **Started:** the hosted
   ...
   Browser Drive/export affordances now route plan-disabled Drive and export
   actions to the same Billing panel instead of opening legacy Drive setup or
   silently hiding the upgrade path.
...
10. Stage 6: add admin/support tooling and production observability.
    **Started:** `GET /api/admin/support/workspaces` is available when
    `PILLAR_PRESS_ADMIN_SECRET` or `PILLAR_PRESS_SUPPORT_SECRET` is configured.
    ...
    diagnose billing blocks without database access. `PILLAR_PRESS_SUPPORT_SECRET`
    is intentionally read-only. `POST /api/admin/support/trials/extend` requires
    `PILLAR_PRESS_ADMIN_SECRET`, extends trial subscriptions by 1-90 days, records
