...
const spendCredits = process.env.PILLAR_PRESS_LIVE_PROVIDER_VERIFY_SPEND_CREDITS === "yes";
const openaiKey = process.env.PILLAR_PRESS_LIVE_OPENAI_API_KEY?.trim();
const xaiKey = process.env.PILLAR_PRESS_LIVE_XAI_API_KEY?.trim() || process.env.PILLAR_PRESS_LIVE_GROK_API_KEY?.trim();
const elevenKey = process.env.PILLAR_PRESS_LIVE_ELEVENLABS_API_KEY?.trim();
const hedraKey = process.env.PILLAR_PRESS_LIVE_HEDRA_API_KEY?.trim();
const openaiBaseUrl = (process.env.PILLAR_PRESS_LIVE_OPENAI_BASE_URL || "https://api.openai.com/v1").replace(/\/+$/, "");
const xaiBaseUrl = (process.env.PILLAR_PRESS_LIVE_XAI_BASE_URL || "https://api.x.ai/v1").replace(/\/+$/, "");
const openaiChatModel = process.env.PILLAR_PRESS_LIVE_OPENAI_CHAT_MODEL?.trim();
const xaiChatModel = process.env.PILLAR_PRESS_LIVE_XAI_CHAT_MODEL?.trim() || "grok-4.3";
const openaiImageModel = process.env.PILLAR_PRESS_LIVE_OPENAI_IMAGE_MODEL?.trim() || "gpt-image-1";
const xaiImageModel = process.env.PILLAR_PRESS_LIVE_XAI_IMAGE_MODEL?.trim() || "grok-imagine-image-quality";
const openaiAudioModel = process.env.PILLAR_PRESS_LIVE_OPENAI_AUDIO_MODEL?.trim() || "gpt-4o-mini-tts";
const ollamaBaseUrl = (process.env.PILLAR_PRESS_LIVE_OLLAMA_BASE_URL || "http://127.0.0.1:11434").replace(/\/+$/, "");
const ollamaModel = process.env.PILLAR_PRESS_LIVE_OLLAMA_MODEL?.trim() || "gemma4:26b-mlx";
const savedDesktopSettingsPath = process.env.PILLAR_PRESS_LIVE_DESKTOP_SETTINGS_PATH?.trim();
...
async function main() {
  ...
  const dataDir = await mkdtemp(join(tmpdir(), "pillar-press-live-provider-"));
  ...
  const desktopSettingsKey = process.env.PILLAR_PRESS_DESKTOP_SETTINGS_KEY?.trim()
  ...
  const child = spawn(nodePath, [serverPath], {
    ...
    env: {
      ...
      PILLAR_PRESS_DESKTOP_SETTINGS_KEY: desktopSettingsKey || "",
  ...
  try {
    ...
    await runCheck(checks, "PROV-005 Ollama task-default utility call", async () => {
      const body = await requestJson(baseUrl, "/api/llm/util", {
        ...
        body: JSON.stringify({
          task: "utility",
          prompt: "Reply with exactly: PILLARPRESS_GEMMA4_OK",
        }),
      });
      const text = (body as { text?: string }).text?.trim();
      if (text !== "PILLARPRESS_GEMMA4_OK") throw new Error(`Unexpected Ollama utility response: ${JSON.stringify(body)}`);
    ...
    if (openaiCredentialSource) {
      ...
      if (spendCredits) {
        ...
        await runCheck(checks, "MEDIA-002 OpenAI saved TTS generation", async () => {
          const body = await requestJson(baseUrl, "/api/hedra/generate", {
            ...
            body: JSON.stringify({
              ...
              prompt: "Pillar Press live audio verification.",
              script: "Pillar Press live audio verification.",
              voiceId: "alloy",
            }),
          });
          const job = (body as { job?: { status?: string; outputUrl?: string; meta?: unknown } }).job;
        ...
        skip(checks, "MEDIA-002 OpenAI image generation", "set PILLAR_PRESS_LIVE_PROVIDER_VERIFY_SPEND_CREDITS=yes to spend provider credits");
        skip(checks, "MEDIA-002 OpenAI saved TTS generation", "set PILLAR_PRESS_LIVE_PROVIDER_VERIFY_SPEND_CREDITS=yes to spend provider credits");
      }
    } else {
      skip(checks, "PROV-004 OpenAI live checks", "PILLAR_PRESS_LIVE_OPENAI_API_KEY not set and no saved desktop OpenAI media key was available");
    }
    ...
    if (xaiCredentialSource) {
      ...
      skip(checks, "PROV-006 xAI/Grok live checks", "PILLAR_PRESS_LIVE_XAI_API_KEY or PILLAR_PRESS_LIVE_GROK_API_KEY not set and no saved desktop xAI key was available");
    }
    if (xaiMediaConfigured || xaiKey) {
      ...
      if (spendCredits) {
        ...
        skip(checks, "MEDIA-002 xAI image generation", "set PILLAR_PRESS_LIVE_PROVIDER_VERIFY_SPEND_CREDITS=yes to spend provider credits");
      }
    }

    if (elevenKey) {
      let voiceId = process.env.PILLAR_PRESS_LIVE_ELEVENLABS_VOICE_ID?.trim();
      ...
      if (spendCredits && voiceId) {
        await runCheck(checks, "MEDIA-002 ElevenLabs saved TTS generation", async () => {
          const body = await requestJson(baseUrl, "/api/hedra/generate", {
            ...
            body: JSON.stringify({
              ...
              prompt: "Pillar Press ElevenLabs verification.",
              script: "Pillar Press ElevenLabs verification.",
              voiceId,
            }),
          });
          const job = (body as { job?: { status?: string; outputUrl?: string; meta?: unknown } }).job;
        ...
        skip(checks, "MEDIA-002 ElevenLabs saved TTS generation", "set PILLAR_PRESS_LIVE_PROVIDER_VERIFY_SPEND_CREDITS=yes to spend provider credits");
      }
    } else {
      skip(checks, "MEDIA-002 ElevenLabs live checks", "PILLAR_PRESS_LIVE_ELEVENLABS_API_KEY not set");
    }
    ...
    if (hedraKey) {
      ...
      skip(checks, "MEDIA-002 Hedra live checks", "PILLAR_PRESS_LIVE_HEDRA_API_KEY not set");
    }
