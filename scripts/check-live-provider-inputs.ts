...
const providerEnvKeys = {
  openai: ["PILLAR_PRESS_LIVE_OPENAI_API_KEY", "OPENAI_API_KEY"],
  xai: ["PILLAR_PRESS_LIVE_XAI_API_KEY", "PILLAR_PRESS_LIVE_GROK_API_KEY", "XAI_API_KEY", "GROK_API_KEY"],
  elevenlabs: ["PILLAR_PRESS_LIVE_ELEVENLABS_API_KEY", "ELEVENLABS_API_KEY"],
  hedra: ["PILLAR_PRESS_LIVE_HEDRA_API_KEY", "HEDRA_API_KEY"],
} as const;
...
function defaultSettingsPath(env: Env) {
  return env.PILLAR_PRESS_LIVE_DESKTOP_SETTINGS_PATH?.trim()
    || join(homedir(), "Library", "Application Support", "com.pillar.press", "desktop-settings.json");
}
...
export function summarizeLiveProviderInputs(env: Env = process.env): ProviderInputSummary {
  ...
  return {
    env: {
      PILLAR_PRESS_LIVE_OPENAI_API_KEY: hasValue(env.PILLAR_PRESS_LIVE_OPENAI_API_KEY),
      PILLAR_PRESS_LIVE_XAI_API_KEY: hasValue(env.PILLAR_PRESS_LIVE_XAI_API_KEY),
      PILLAR_PRESS_LIVE_GROK_API_KEY: hasValue(env.PILLAR_PRESS_LIVE_GROK_API_KEY),
      PILLAR_PRESS_LIVE_ELEVENLABS_API_KEY: hasValue(env.PILLAR_PRESS_LIVE_ELEVENLABS_API_KEY),
      PILLAR_PRESS_LIVE_HEDRA_API_KEY: hasValue(env.PILLAR_PRESS_LIVE_HEDRA_API_KEY),
      PILLAR_PRESS_LIVE_PROVIDER_VERIFY_SPEND_CREDITS: env.PILLAR_PRESS_LIVE_PROVIDER_VERIFY_SPEND_CREDITS === "yes",
      PILLAR_PRESS_LIVE_DESKTOP_SETTINGS_PATH: hasValue(env.PILLAR_PRESS_LIVE_DESKTOP_SETTINGS_PATH),
    },
    savedSettings: {
      path,
    ...
    releaseBlockerReadiness: {
      ...
      media002SpendFlag: env.PILLAR_PRESS_LIVE_PROVIDER_VERIFY_SPEND_CREDITS === "yes",
      media002Providers: {
...
function main() {
  ...
  if (releaseBlockerReadiness.prov004OpenAI === "missing") {
    console.error("PROV-004 still needs PILLAR_PRESS_LIVE_OPENAI_API_KEY or a saved desktop OpenAI key for live OpenAI model/test evidence.");
  }
  if (!releaseBlockerReadiness.media002SpendFlag) {
    console.error("MEDIA-002 spend-credit generation still needs PILLAR_PRESS_LIVE_PROVIDER_VERIFY_SPEND_CREDITS=yes.");
  }
}
